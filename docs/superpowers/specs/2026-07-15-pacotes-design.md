...
1. **Frontend:** nova rota `/voucher-preview/pacote/:id` → componente que faz `GET /api/packages/:id`, extrai `.flights`, e **reusa o dispatch de template de voo existente** (`STYLES` map → `VoucherCanonicalV1`/`VoucherCompactoV1`) passando esse unified. (O template de voo não sabe de onde vieram os dados — só recebe `data`.) **Gotcha:** o gate de preview standalone em `App.jsx` casa hoje só `^/voucher-preview/(\d+)` (numérico) — estender o regex pra aceitar `/voucher-preview/pacote/:id` também.

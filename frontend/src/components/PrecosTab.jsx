    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800/50 p-4 rounded-xl flex items-center gap-3">
          ...
          <div><div className="text-xs text-slate-400 font-medium">Total de Voos</div><div className="text-lg font-bold text-white">{flights.length}</div></div>
        </div>
        <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800/50 p-4 rounded-xl flex items-center gap-3">
          ...
          <div><div className="text-xs text-slate-400 font-medium">Checks Concluídos</div><div className="text-lg font-bold text-white">{flights.filter(f=>f.check_diario).length} / {flights.length}</div></div>
        </div>
        <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800/50 p-4 rounded-xl flex items-center gap-3">
          ...
          <div className="w-full"><div className="text-xs text-slate-400 font-medium">Ordenar por</div>
            <select value={sortBy} onChange={e=>setSortBy(e.target.value)} className="bg-transparent border-0 text-sm font-bold text-white focus:ring-0 focus:outline-none cursor-pointer w-full mt-0.5">
              <option value="manual" className="bg-slate-900">Manual ↕</option>
              <option value="priority" className="bg-slate-900">Prioridade ★</option>
              <option value="date" className="bg-slate-900">Data Viagem 📅</option>
            </select>
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {isLoading ? (
                <tr><td colSpan={colCount} className="px-6 py-12 text-center text-slate-400">
                ...
                <tr><td colSpan={colCount} className="px-6 py-12 text-center text-slate-400">
                  <Plane className="w-12 h-12 mx-auto text-slate-600 mb-3 opacity-50" /><p>Nenhum voo sendo monitorado.</p>
                </td></tr>
                ...
                <tr key={flight.id} draggable={sortBy==='manual'} onDragStart={e=>handleDragStart(e,index)} onDragOver={e=>handleDragOver(e,index)} onDragEnd={handleDragEnd}
                  className={`hover:bg-slate-800/30 transition-all group ${sortBy==='manual'?'cursor-move':''} ${draggedIndex===index?'opacity-30 bg-indigo-500/10':''}`}>
                  {sortBy==='manual' && <td className="px-4 py-4 text-center text-slate-500 group-hover:text-slate-300"><GripVertical className="w-5 h-5 mx-auto cursor-grab" /></td>}
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="font-semibold text-slate-200">{flight.cliente}</span>
                      <span className="text-xs text-slate-400 flex items-center gap-1 mt-0.5">
                        <Users className="w-3.5 h-3.5 text-slate-500" /> {flight.quantidade_pax||1} {(flight.quantidade_pax||1)===1?'pax':'pax'}
                        {flight.email_cliente && <Mail className="w-3 h-3 text-slate-500 ml-1" />}
                        {flight.telegram_chat_id && <MessageSquare className="w-3 h-3 text-slate-500" />}
                      </span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-slate-300">{flight.mes_viagem}</td>
                  ...
                  <td className="px-6 py-4 font-mono text-emerald-400">{fmt(flight.preco_esperado)}</td>
                  <td className="px-6 py-4">
                    {flight.preco_atual != null ? (
                      <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                          <span className={`font-mono font-semibold ${flight.preco_atual <= flight.preco_esperado ? 'text-emerald-400' : 'text-slate-300'}`}>{fmt(flight.preco_atual)}</span>
                          {flight.preco_atual <= flight.preco_esperado && <span className="text-[10px] font-bold bg-emerald-500/20 text-emerald-400 px-1.5 py-0.5 rounded border border-emerald-500/30">✓ ALERTA</span>}
                        </div>
                        {flight.ultima_verificacao && <span className="text-[10px] text-slate-500 mt-0.5">{timeAgo(flight.ultima_verificacao)}</span>}
                      </div>
                    ) : <span className="text-slate-600">—</span>}
                  </td>
                  <td className="px-6 py-4 text-center">
                    <button onClick={()=>toggleCheck(flight)} className={`transition-colors rounded-full p-1 cursor-pointer ${flight.check_diario?'text-indigo-400 hover:text-indigo-300 bg-indigo-500/10':'text-slate-500 hover:text-slate-400 hover:bg-slate-800'}`}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-end gap-1.5 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={()=>checkNow(flight.id)} disabled={checkingId===flight.id} className="p-2 text-slate-400 hover:text-amber-400 bg-slate-800 hover:bg-amber-500/20 rounded-lg transition-colors cursor-pointer disabled:opacity-50" title="Verificar agora">
                        <RefreshCw className={`w-4 h-4 ${checkingId===flight.id?'animate-spin':''}`} />
                      </button>
                      <a href={flight.link_voo} target="_blank" rel="noopener noreferrer" className="p-2 text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors" title="Google Voos"><ExternalLink className="w-4 h-4" /></a>
                      <button onClick={()=>openModal(flight)} className="p-2 text-slate-400 hover:text-blue-400 bg-slate-800 hover:bg-blue-500/20 rounded-lg transition-colors cursor-pointer" title="Editar"><Edit2 className="w-4 h-4" /></button>
                      <button onClick={()=>deleteFlight(flight.id)} className="p-2 text-slate-400 hover:text-red-400 bg-slate-800 hover:bg-red-500/20 rounded-lg transition-colors cursor-pointer" title="Excluir"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-slate-900/60 backdrop-blur-xl border border-slate-700/50 shadow-2xl rounded-2xl overflow-hidden modal-animate max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center p-6 border-b border-slate-700/50">
              <h2 className="text-xl font-semibold text-white">{editingFlight?'Editar Voo':'Adicionar Novo Voo'}</h2>
              <button onClick={closeModal} className="text-slate-400 hover:text-white transition-colors cursor-pointer"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="col-span-1 space-y-1">
                  <label className="text-sm font-medium text-slate-300 flex items-center gap-2"><Users className="w-4 h-4" /> Pax</label>
                  <input type="number" min="1" {...register('quantidade_pax',{required:true,min:1})} className={inputCls} placeholder="1" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-sm font-medium text-slate-300 flex items-center gap-2"><Calendar className="w-4 h-4" /> Mês da Viagem</label>
                  <input {...register('mes_viagem',{required:true})} className={inputCls} placeholder="Ex: Janeiro/2025" />
                  {errors.mes_viagem && <span className="text-xs text-red-400">Obrigatório</span>}
                </div>
                <div className="space-y-1">
                  <label className="text-sm font-medium text-slate-300 flex items-center gap-2"><AlertCircle className="w-4 h-4" /> Prioridade</label>
                  ...
                  {errors.prioridade && <span className="text-xs text-red-400">Obrigatório</span>}
                </div>
              </div>
              <div className="space-y-1">
                <label className="text-sm font-medium text-slate-300 flex items-center gap-2"><DollarSign className="w-4 h-4" /> Preço Esperado (R$)</label>
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-slate-300 flex items-center gap-2"><LinkIcon className="w-4 h-4" /> Link do Google Voos</label>
                <input type="url" {...register('link_voo',{required:true})} className={inputCls} placeholder="https://www.google.com/travel/flights..." />
                {errors.link_voo && <span className="text-xs text-red-400">Link obrigatório</span>}
              </div>

              {/* New fields */}
              <div className="border-t border-slate-700/50 pt-4 mt-2">
                <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-3">Notificações & Status</p>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    {errors.email_cliente && <span className="text-xs text-red-400">E-mail inválido</span>}
                  </div>
                  <div className="space-y-1">
                ...
                <div className="mt-3 space-y-1">
                  ...
                  <select {...register('status')} className={`${inputCls} appearance-none`}>
                    ...
                    <option value="passagem comprada">Passagem Comprada</option>
                  </select>
                </div>
              </div>

              <div className="pt-4 flex justify-end gap-3 border-t border-slate-700/50 mt-6">
                <button type="button" onClick={closeModal} className="px-5 py-2.5 text-sm font-medium text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors cursor-pointer">Cancelar</button>

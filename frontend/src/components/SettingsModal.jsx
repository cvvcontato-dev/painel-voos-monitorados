...
export default function SettingsModal({ isOpen, onClose, onToast }) {
  ...
  const inputCls = "w-full bg-slate-800/50 border border-slate-700 text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all px-4 py-2.5 rounded-lg text-sm";
  const labelCls = "text-xs font-medium text-slate-400 uppercase tracking-wider";

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm" onClick={onClose}></div>
      <div className="relative w-full max-w-lg bg-slate-900/90 backdrop-blur-xl border border-slate-700/50 shadow-2xl rounded-2xl overflow-hidden modal-animate max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <label className={labelCls}>Bot Token {settings.telegram_bot_token_set && <span className="text-emerald-400 ml-1">✓ configurado</span>}</label>
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowToken(!showToken)} className="absolute right-3 top-2.5 text-slate-400 hover:text-white cursor-pointer">
          ...
          <div>
            ...
            <div className="space-y-3">
              <div><label className={labelCls}>Usuário {settings.email_user_set && <span className="text-emerald-400 ml-1">✓</span>}</label>
              ...
              <div><label className={labelCls}>Senha {settings.email_pass_set && <span className="text-emerald-400 ml-1">✓</span>}</label>
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-2.5 text-slate-400 hover:text-white cursor-pointer">
        ...
        <div className="p-6 border-t border-slate-700/50 flex justify-end gap-3">
          <button onClick={onClose} className="px-5 py-2.5 text-sm font-medium text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors cursor-pointer">Cancelar</button>
